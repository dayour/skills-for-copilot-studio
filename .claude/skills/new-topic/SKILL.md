...
3. **MANDATORY: Verify ALL `kind:` values against the schema** before writing them:
   ```bash
   python scripts/schema-lookup.py kinds                    # List all valid kind values
   python scripts/schema-lookup.py resolve AdaptiveDialog   # Resolve trigger structure
   python scripts/schema-lookup.py resolve <TriggerType>    # Resolve specific trigger
   python scripts/schema-lookup.py search <ActionKind>      # Verify an action kind exists
   ```
...
8. **MANDATORY: Validate the generated file** after saving:
   ```bash
   python scripts/schema-lookup.py validate <saved-file.yml>
   ```
