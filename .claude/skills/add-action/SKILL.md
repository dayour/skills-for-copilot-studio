...
```bash
python scripts/schema-lookup.py summary TaskDialog
python scripts/schema-lookup.py summary InvokeConnectorTaskAction
```
...
2. **Read the action catalog** to find available actions:
   ```
   Read: .claude/skills/add-action/catalog.md
   ```
...
4. **Read the sample file** from `templates/actions/samples/<sample-file>` as indicated in the catalog.
